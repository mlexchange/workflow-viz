...
def schedule_prefect_flow(
    ...
    if "input_data_uri" in parameters:
        ...
        flow_run_name = None
    flow_run_id = asyncio.run(_schedule(deployment_name, flow_run_name, parameters))
    return flow_run_id


async def _check(flow_run_id):
    async with get_client() as client:
...
def check_status_flow_run(flow_run_id):
    """Checks on the current status of the flow with the given id."""
    return asyncio.run(_check(flow_run_id))
